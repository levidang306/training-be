...
export class PaginationUtil {
  ...
  static async paginateWithCursor<T>(
    ...
    const nextCursor = hasMore && items.length > 0 
      ? (items[items.length - 1] as any)[orderBy] 
      : null;
...
export class PerformanceQueryRunner extends QueryRunner {
  async query(query: string, parameters?: any[]): Promise<any> {
    ...
    try {
      ...
      const tableMatch = query.match(/FROM\s+([^\s]+)/i) || query.match(/UPDATE\s+([^\s]+)/i) || query.match(/INSERT\s+INTO\s+([^\s]+)/i);
...
export class MemoryMonitor {
  ...
  private startMonitoring(): void {
    ...
    setInterval(() => {
      ...
      if (heapUsedMB > 500) { // 500MB warning threshold
      ...
      const obs = new PerformanceObserver((list) => {
        ...
        entries.forEach((entry) => {
          if (entry.entryType === 'gc') {
            this.gcDurationHistogram
              .labels(entry.detail?.kind?.toString() || 'unknown')
